...
let inputLibOpenflPath = _path.resolve(baseDir, inputLibDirname, 'motion');
let outputLibOpenflPath = _path.resolve(baseDir, outputLibDirname, 'motion');
  ...
  return globby([_path.resolve(inputLibOpenflPath, '**/index.js')]).then((paths) => {
    ...
    for (let path of paths) {
      ...
      if (path.indexOf('index.js') > -1) {
        delayFS.writeFileSyncUTF8(esmFilePath, result, {tags: 'index', createOnly: true});
      } else {
      ...
      if (path.indexOf('index.js') > -1) {
        delayFS.writeFileSyncUTF8(dTSFilePath, result, {tags: 'index', createOnly: true});
      } else {
...
function createEsmModule(content, filePath) {
  ...
  result = result.replace(/^var (.+?) = require\s*\(['"](.+?)['"]\)\.default;/gm, 'import { default as $1 } from "$2";');
  
  // Replace
  // function openfl_display_DisplayObject() {return require("./../../openfl/display/DisplayObject");}
  // with
  // import { default as openfl_display_DisplayObject } from "./../../openfl/display/DisplayObject";
  result = result.replace(/^function (.+?)\(\) {return require\s*\(['"]\.\/(.+?)['"]\);}/gm, 'import { default as $1 } from "./$2";');
  ...
  // with
  // import { default as openfl_display_DisplayObject } from "openfl/display/DisplayObject";
  result = result.replace(/^function (.+?)\(\) {return \$import\s*\(require\s*\(['"](.+?)['"]\)\);}/gm, 'import { default as $1 } from "$2";');
...
function createIndexModules(content, filePath) {
  ...
  result = result.replace(/^\s*(.+?): require\s*\(["'](.+?)["']\).*/gm, (match, p1, p2) => {
  ...
  result = result.replace(/(?:module\.)?exports\.default = (?:module\.exports\.)?(.+)/g, 'export default $1');
